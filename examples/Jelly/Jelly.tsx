...
interface JellyProps {
  ...
  mousePos?: SharedValue<{ x: number; y: number }>;
}

export default function Jelly({ isDragging, mousePos }: JellyProps) {
  const ref = useWebGPU(async ({ context, device, presentationFormat }) => {
    ...
    const sliderSdf3D = (position: d.v3f) => {
      ...
      if (poly2D.t > 0.94) {
        ...
        const body =
          sdf.opExtrudeZ(position, poly2D.distance, LINE_HALF_THICK) -
          LINE_RADIUS;
        finalDist = body;
      }
    ...
    const getFakeShadow = (position: d.v3f, lightDir: d.v3f): d.v3f => {
      ...
      if (position.y < -GroundParams.groundThickness) {
        ...
        const finalUV = d.vec2f(
          (position.x - position.z * lightDir.x * std.sign(lightDir.z)) * 0.5 +
            0.5,
    ...
    const calculateLighting = (
      ...
      return std.saturate(
        directionalLight.add(ambientLight).add(finalSpecular),
      );
    };
    ...
    const renderBackground = (
      ...
      if (
        ...
        const uvX_orig =
          ((hitPosition.x + offsetX + highlightWidth * 2) / highlightWidth) *
          0.5;
        const uvZ_orig =
          ((hitPosition.z + offsetZ + highlightHeight * 2) / highlightHeight) *
          0.5;
        ...
        const edgeFade =
          std.saturate(fadeX) * std.saturate(fadeZ) * fadeStretch;
    ...
    const rayMarch = (rayOrigin: d.v3f, rayDirection: d.v3f, uv: d.v2f) => {
      ...
      for (let i = 0; i < MAX_STEPS; i++) {
        ...
        const currentPosition = rayOrigin.add(
          rayDirection.mul(distanceFromOrigin),
        );
        ...
        if (hitInfo.distance < SURF_DIST) {
          const hitPosition = rayOrigin.add(
            rayDirection.mul(distanceFromOrigin),
          );
    ...
    const eventHandler = new EventHandler(width / PixelRatio.get());
