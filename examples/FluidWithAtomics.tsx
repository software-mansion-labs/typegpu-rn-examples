  ...
  const ref = useWebGPU(({ context, device, presentationFormat }) => {
    ...
    const addToCell = tgpu['~unstable']
      ...
      .does((x, y, value) => {
        ...
        const newWaterLevel = std.min(
          waterLevel + value,
          MAX_WATER_LEVEL.value,
        );
        std.atomicAdd(
    ...
    const subtractFromCell = tgpu['~unstable']
      ...
      .does((x, y, value) => {
        const cell = getCellNext(x, y);
        const waterLevel = cell & MAX_WATER_LEVEL.value;
        const newWaterLevel = std.max(
          waterLevel - std.min(value, waterLevel),
          0,
        );
        std.atomicSub(
          nextStateStorage.value[getIndex(x, y)],
      ...
      .does((x, y) => {
        ...
        if (!isWall(x, y + 1)) {
          ...
          if (stable < remainingWater) {
            const flow = std.min(
              remainingWater - stable,
              viscosityUniform.value,
            );
            subtractFromCell(x, y, flow);
            addToCell(x, y + 1, flow);
            remainingWater -= flow;
    ...
    const vertex = tgpu['~unstable']
      ...
      .does((input) => {
        ...
        const y =
          ((d.f32(
            (input.idx - (input.idx % w)) / w + d.u32(input.squareData.y),
          ) /
            d.f32(h) -
            0.5) *
            2 *
